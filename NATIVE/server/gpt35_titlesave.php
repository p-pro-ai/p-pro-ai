...
if (isset($input)) {
  ...
  $sql = "SELECT title FROM chatGPT WHERE session_id=" . $random_secure;
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $title = $row['title'];
  ...
  $sql = "INSERT INTO chatGPT (IP, datetime_1, message_to, answer_res, session_id, user_email, model, title)
  VALUES ('$myip', '$datetime', '$input', '$message_to', '$random_secure', '$email_addr', 'gpt-3.5-turbo', '$title')";
  $conn->query($sql);
  ...
  echo json_encode($return);
    ...
    $messages[] = array("role" => "user", "content" => 'Think of a title for our conversation, where I ask you - "'.$input.'"');
    $data = array(
        "model" => "gpt-3.5-turbo",
        "messages" => $messages
      );
      ...
      $title = mysqli_real_escape_string($conn, $data['choices'][0]['message']['content']);



      $messages1[] = array("role" => "user", "content" => $input);
