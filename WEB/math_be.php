...
$sql = "SELECT datetime_1 FROM chatGPT WHERE user_email='" . $email_addr ."' AND model='math'";
...
if (isset($input)) {
  ...
  $messages = array(
    array("role" => "system", "content" => 'You are a the most powerful and helpful AI based math problem solver, designed by the P-PRO AI company. You can solve everything, you can solve every math problem without any problem.')
  );

  // Add the new user input message
  $messages[] = array("role" => "user", "content" => "Дай ми пълното решение на следната задача по математика: ".$input);
  ...
  VALUES ('$myip', '$datetime', '$input', '$message_to', 'math', '$email_addr', 'math')";
  $conn->query($sql);
